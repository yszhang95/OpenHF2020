                ...
                std::map<std::string, std::string> strs)
{
  ...
  auto result = fitSideband(*cand_mass, *dsData, myws, par, opts, strs);
  std::cout << "\n";
  ...
  ofileName.replace(pos, 5, "_" + strs.at("order") + "_sideband_postfit.root");
...
void fitEachBin(const std::string& configName, const int order)
{
  ...
  strs["outdir"] = "./sideband/";
  strs["order"] = "order" + std::to_string(order);

  const char* weightstr = mycuts._useWeight ? "_weighted" : "";
  strs["path_name_log"] = Form("sideband_output/LamCFit_pT%.1fto%.1f_Ntrk%dto%d_order%d%s.log",
      pTMin, pTMax, NtrkMin, NtrkMax, order, weightstr);
  strs["fig_path"] = Form("sideband_output/pT%.1fto%.1f_Ntrk%dto%d_order%d%s",
      pTMin, pTMax, NtrkMin, NtrkMax, order, weightstr);
...
void fitSideBand(const int order = 3)
{
