                ...
                std::map<std::string, std::string> strs)
{
  ...
  fitGausCB(*cand_mass, *dsMCSignal, par, strs);
  ...
  auto result = fitLamCGausCB(*cand_mass, *dsData, myws, par, opts, strs);
  std::cout << "\n";
  ...
  ofileName.replace(pos, 5, "_gausCB_postfit.root");
...
void fitEachBin(const std::string& configName)
{
  ...
  strs["path_name_log"] = Form("dz1p0_output_gausCB/LamCFit_pT%.1fto%.1f_Ntrk%dto%d_gausCB%s.log",
      pTMin, pTMax, NtrkMin, NtrkMax, weightstr);
  strs["fig_path"] = Form("dz1p0_output_gausCB/pT%.1fto%.1f_Ntrk%dto%d_gausCB%s",
      pTMin, pTMax, NtrkMin, NtrkMax, weightstr);
...
void fitGausCB()
{
  ...
  for (unsigned int ibin=5; ibin<6; ++ibin) {
    ...
    for (unsigned int ipt=1; ipt<2; ++ipt) {
      ...
      auto configName = ::string_format("dz1p0_%s_gausCB/pT%.0fto%.0f_Ntrk%dto%d.conf",
                                        useWeight, pts[ipt], pts[ipt+1], Ntrks[ibin], Ntrks[ibin+1]);
