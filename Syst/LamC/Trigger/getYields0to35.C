...
void getYields0to35()
{
  ...
  TFile output("yields_weighted_Ntrk0to35.root", "recreate");
  TH1D* h[3];
  for (int i=0; i<3; ++i) {
    const auto weightType = weightTypes[i];
    // TFile ofile(::Form("yields_%s.root", weightType.c_str()), "recreate");
    h[i] = new TH1D(::Form("h%s", weightType.c_str()), ::Form("Ntrk2to35_%s;p_{T} (GeV);1/N_{evt} dN^{raw}/dp_{T} (GeV^{-1})", weightType.c_str()), nPt, pts);
    for (int ipt=0; ipt<nPt; ++ipt) {
      const int itrk = 0;
