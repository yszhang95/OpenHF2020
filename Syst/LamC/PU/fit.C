...
void fitEachBin(const std::string& configName, const std::string& trig)
{
  ...
  strs["path_name_log"] = Form("%s_output/LamCFit_pT%.1fto%.1f_Ntrk%dto%d%s.log",
      trig.c_str(), pTMin, pTMax, NtrkMin, NtrkMax, weightstr);
...
void fit(const std::string trig="dz1p0")
{
  ...
  for (unsigned int ibin=0; ibin<nbins; ++ibin) {
    ...
    for (unsigned int ipt=1; ipt<nPt; ++ipt) {
      ...
      auto configName = ::string_format("${OPENHF2020TOP}/Spectra/ProduceWorkspace/LamC/%s_%s/pT%.0fto%.0f_Ntrk%dto%d.conf", trig.c_str(), useWeight, pts[ipt], pts[ipt+1], Ntrks[ibin], Ntrks[ibin+1]);
