...
void getYields(std::string trig="gplus")
{
  ...
  for (int itrk=0; itrk<ntrks; ++itrk) {
    ...
    for (int ipt=0; ipt<nPt; ++ipt) {
      ...
      //                                         useWeight);
      std::string configName = ::string_format("configs_%s/D0wKKPiPi_pT%.0fto%.0f_Ntrk%dto%d_std.txt", trig.c_str(), pts[ipt], pts[ipt+1], Ntrks[itrk], Ntrks[itrk+1]);
      ...
      names["outdir"] = ::Form("./output_%s/", trig.c_str());
