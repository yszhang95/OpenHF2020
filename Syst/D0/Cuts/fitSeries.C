                ...
                std::map<std::string, std::string> strs)
{
  ...
  const auto fig_dir = gSystem->DirName(fig_path.c_str());
  if (gSystem->AccessPathName(out_dir)) {
    gSystem->mkdir(out_dir, kTRUE);
  }
  if (gSystem->AccessPathName(fig_dir)) {
    gSystem->mkdir(fig_dir, kTRUE);
  }
  ...
  mass->setRange("left", 1.725, 1.8);
  mass->setRange("peak", 1.83, 1.91);
  mass->setRange("right", 1.91, 2.0);
  ...
  strs["dsMCSignal"] = "dsMCSignal" + MClabel;
  strs["dsMCSwap"] = "dsMCSwap" + MClabel;
  strs["dsMCAll"] = "dsMCAll" + MClabel;
  strs["dsMCKK"] = "dsMCKK" + MClabel;
  strs["dsMCPiPi"] = "dsMCPiPi" + MClabel;
  strs["dsData"] = "ds" + std::string(wsName.begin()+2, wsName.end());
  ...
  D0Fitter fitter(par);
  int status = -1;
  auto result = fitter.fitD0(*mass, myws, opts, strs);
...
void fitEachMult(int NtrkMin, int NtrkMax, const double mvaCut)
{
  // for loop
  std::map<std::string, std::string> strs;
  const int nPt = 8;
  ...
  // for (int ipt=0; ipt<1; ++ipt) {
    ...
    // std::string configName = ::string_format("configs_%s/D0wKKPiPi_pT%.0fto%.0f_Ntrk%dto%d_%s.txt", cut.c_str(), pts[ipt], pts[ipt+1], NtrkMin, NtrkMax, cut.c_str());
    // std::string configName = ::string_format("D0wKKPiPi_Ntrk%dto%d_std.txt", NtrkMin, NtrkMax);
    std::string configName = ::string_format("D0woKKPiPi_Ntrk%dto%d.txt", NtrkMin, NtrkMax);
    std::cout << configName << "\n";
    ...
    mycuts._mvaCut = mvaCut;

    auto names = ::getNames(configs, mycuts);
    strs["outdir"] = "output/files/";
    if (*strs.at("outdir").rend() != '/') strs.at("outdir") += "/";
    strs["path_name_log"] = Form("output/log_%.2f/D0Fit_pT%.1fto%.1f_Ntrk%dto%d_%.2f.log",
                                     mvaCut, pts[ipt], pts[ipt+1], NtrkMin, NtrkMax, mvaCut);
    strs["fig_path"] = Form("output/fig_%.2f/pT%.1fto%.1f_Ntrk%dto%d_%.2f",
                                mvaCut, pts[ipt], pts[ipt+1], NtrkMin, NtrkMax, mvaCut);
...
void fitSeries()
{
  ...
  for (unsigned int ibin=0; ibin<nbins; ++ibin) {
      for (int iMVA=0; iMVA!=nMVA+1; ++iMVA) {
        ...
        fitEachMult(Ntrks[ibin], Ntrks[ibin+1], mvaCut);
      }
  }
}
