...
def plotKs(fname):
  ...
  for t, ls, ms in zip(Regions, LineStyle, Markers):
    for e, col in zip(Charges, Colors):
      ...
      hdR[t+e].Draw("ESAME");
  cdR.BuildLegend()
  cdR.Print('plots/KsdR_'+fname.replace('.root', '.png'))
  cdR.Print('plots/KsdR_'+fname.replace('.root', '.pdf'))
  ...
  for t, ls, ms in zip(Regions, LineStyle, Markers):
    for e, col in zip(Charges, Colors):
      ...
      hdPt[t+e].Draw("ESAME");
  cdPt.BuildLegend()
  cdPt.Print('plots/KsdPt_'+fname.replace('.root', '.png'))
  cdPt.Print('plots/KsdPt_'+fname.replace('.root', '.pdf'))
  ...
  cChi2.BuildLegend()
  cChi2.Print('plots/KsChi2_'+fname.replace('.root', '.png'))
  cChi2.Print('plots/KsChi2_'+fname.replace('.root', '.pdf'))
  ...
  cdRCut.Print('plots/KsdPtdRCut_'+fname.replace('.root', '.png'))
  cdRCut.Print('plots/KsdPtdRCut_'+fname.replace('.root', '.pdf'))
  ...
  cdPtCut.BuildLegend()
  cdPtCut.Print('plots/KsdRdPtCut_'+fname.replace('.root', '.png'))
  cdPtCut.Print('plots/KsdRdPtCut_'+fname.replace('.root', '.pdf'))

  cMass = r.TCanvas("cmass", '', 500, 400)
  hmass = f.Get("hKsMass")
  hmass.Draw()
  cMass.Print('plots/KsMass.pdf')
  cMass.Print('plots/KsMass_'+fname.replace('.root', '.png'))
  cMass.Print('plots/KsMass_'+fname.replace('.root', '.pdf'))

#plotKs('PA8TeV_pPb_LamC_MC_20210302_sortBydPtsortBydR_KsInfoPostfit.root')
plotKs('PA8TeV_pPb_LamC_MC_20210302_sortBydPtsortBydR_KsInfoPrefit.root')
