...
void getYields()
{
  ...
  const unsigned int nPt = sizeof(pts)/sizeof(double);
  // forget 2 -- 3
  for (int ipt=1; ipt<nPt; ++ipt) {
    for (int itrk=0; itrk<ntrks; ++itrk) {
      if (ipt == 0 && itrk == ntrks-1 ) continue;
      const char* useWeight = itrk == ntrks-1 ? "weighted" : "unweighted";
      std::string configName = ::string_format("%s_%s/pT%.0fto%.0f_Ntrk%dto%d.conf", "dzp10", useWeight, pts[ipt], pts[ipt+1], Ntrks[itrk], Ntrks[itrk+1]);
      double yields = 0;
      double err = 0;
