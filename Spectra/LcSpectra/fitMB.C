                ...
                std::map<std::string, std::string> strs)
{
  ...
  cand_mass->setMin(2.1);
  cand_mass->setMax(2.47);
...
void fitEachBin(const std::string& configName)
{
  ...
  strs["path_name_log"] = Form("dz1p0_output/LamCFit_pT%.1fto%.1f_Ntrk%dto%d%s.log",
      pTMin, pTMax, NtrkMin, NtrkMax, weightstr);
  strs["fig_path"] = Form("dz1p0_output/pT%.1fto%.1f_Ntrk%dto%d%s",
      pTMin, pTMax, NtrkMin, NtrkMax, weightstr);
...
void fitMB(const float pTMin, const float pTMax)
{
  const char* useWeight = "unweighted";
  auto configName = ::string_format("${OPENHF2020TOP}/Spectra/ProduceWorkspace/LamC/dz1p0_%s/pT%.0fto%.0f_MB.conf", useWeight, pTMin, pTMax);
