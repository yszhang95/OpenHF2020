...
void proj1D(std::string configName)
{
  ...
  hists["ProtonPtVsMass"].insert({"data", infile.Get<TH2D>("ProtonPtVsMass")});
  hists["ProtonPtVsMass"].insert({"MC", infileMC.Get<TH2D>("ProtonPtVsMassMC")});
  hists["ProtonPVsMass"].insert({"data", infile.Get<TH2D>("ProtonPVsMass")});
  hists["ProtonPVsMass"].insert({"MC", infileMC.Get<TH2D>("ProtonPVsMassMC")});
  hists["ProtonEtaVsMass"].insert({"data", infile.Get<TH2D>("ProtonEtaVsMass")});
  hists["ProtonEtaVsMass"].insert({"MC", infileMC.Get<TH2D>("ProtonEtaVsMassMC")});
  hists["ProtonDeDxVsMass"].insert({"data", infile.Get<TH2D>("ProtonDeDxVsMass")});
  hists["ProtonDeDxVsMass"].insert({"MC", infileMC.Get<TH2D>("ProtonDeDxVsMassMC")});
  hists["CosAngleVsMass"].insert({"data", infile.Get<TH2D>("CosAngleVsMass")});
  hists["CosAngleVsMass"].insert({"MC", infileMC.Get<TH2D>("CosAngleVsMassMC")});
  ...
  hists_names["ProtonPtVsMass"] = "ProtonPt";
  hists_names["ProtonPVsMass"] = "ProtonP";
  hists_names["ProtonEtaVsMass"] = "ProtonEta";
  hists_names["CosAngleVsMass"] = "CosAngle";

  hists_titles["ProtonPtVsMass"] = "ProtonPt";
  hists_titles["ProtonPVsMass"] = "ProtonP";
  hists_titles["ProtonEtaVsMass"] = "ProtonEta";
  hists_titles["CosAngleVsMass"] = "CosAngle";

  // hists_bins["ProtonDeDxVsMass"] = {};
  // hists_bins["ProtonPtVsMass"] = {0, 1.5, 3, 5, 7, 10}; // pT 6 -- 10
  // hists_bins["ProtonPVsMass"] = {0, 1.5, 3, 5, 7, 10};
  // hists_bins["ProtonEtaVsMass"] = {-2.4, -1, 0, 1, 2.4}; // pT 6 -- 10
  // hists_bins["ProtonEtaVsMass"] = {-2.4, -1.4, -0.6, 0, 0.6, 1.4, 2.4};
  // hists_bins["ProtonEtaVsMass"] = {-2.4, -1.4, -0.7, 0, 0.7, 1.4, 2.4}; // pT 6 -- 10
  // hists_bins["CosAngleVsMass"] = {-1, -0.95, -0.8, 0, 0.8, 0.9, 0.95, 1}; // pT 6 -- 10
  ...
  for (auto& e : hists) {
    if (e.first == "ProtonDeDxVsMass") continue;

    std::cout << "Analyzing the variable " << e.first << "\n";

    auto hMC = proj1D(e.second.at("MC"), hists_names.at(e.first) + "_mass", -1E4, 1E4);
    ...
    auto hMC_var = e.second.at("MC")->ProjectionY((hists_names.at(e.first)+"_MC").c_str(), 0, 1000000);
    auto hMC_rebin = hMC_var->Rebin(binning.size()-1, ::Form("%s_rebin", hMC_var->GetName()), binning.data());
...
void proj1D_NoMVA()
{
  ...
  // for (int ipt=nPt-2; ipt<nPt-1; ++ipt) {
    for (int itrk=0; itrk<nNtrkofflines; ++itrk) {
      std::cout << "Analyzing " << pts[ipt] << " " << pts[ipt+1] << " Ntrkoffline " << Ntrkofflines[itrk] << " " << Ntrkofflines[itrk] << "\n";
...
void proj1D_NoMVA(const float pTMin, const float pTMax)
{
