...
def getArrs(arr):
    ...
    x = np.array([(x1-x2, x1+x3) for x1, x2, x3 in zip(arr[:,0], arr[:, 2], arr[:, 3])])
    output["x"] = x.flatten()
...
yvalues[r'$-3.46<y_{cm}<-2.46$'] = np.loadtxt('output_pp_8p16TeV/pTspectra_y-3p46to-2p46.txt', delimiter=' ')
...
yvalues[r'$1.54<y_{cm}<2.54$'] = np.loadtxt('output_pp_8p16TeV/pTspectra_y1p54to2p54.txt', delimiter=' ')
