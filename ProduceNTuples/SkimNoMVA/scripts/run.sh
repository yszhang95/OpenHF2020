...
# the first parameter is always the jobID
jobId=$1
...
if [[ "$boost" = "Pbp" ]]; then
    etaFlag="--flipEta"
    etaPostfix="_etaFlipped"
fi
ptmin_int="${ptmin%.*}"
ptmin_decimal="${ptmin#*.}"
ptminstr="${ptmin_int}p${ptmin_decimal}"

echo "Eta flag is ${etaFlag}"

${OPENHF2020TOP}/Training/Macros/skimTree.py -i ${inputfiles} \
                --pTMin ${ptmin} --wantAbort \
                --selectDeDx ${etaFlag} \
...
outputFile="input_files_lambdacAna_AllEntries_pT${ptminstr}to20p0_yAbs1p0_absMassDiff20p000_LamCKsP${etaPostfix}.root"
mv $outputFile $here/
mycode2=$?
